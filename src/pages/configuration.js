...
export default class Configuration extends React.Component {
	...
	getLastWins = async () => {
		console.log('quantas vezes isso é chamado')
		const {
			user: {
				userReducer: {
					userId,
	...
	getLastResolution = (lastPlays) => {
		...
		const a = timeUntilResolution + 2000
	...
	getWinsArray = async playsArray => {
		let resultsObj = {};
		let retryTime = 5000;

		while (Object.keys(resultsObj).length === 0 || resultsObj.open_options.length) {
			resultsObj = await this.getResultsObj()

			if (resultsObj.open_options.length) {
				retryTime = this.getLastResolution(resultsObj)
				console.log({ retryTime, resultsObj })
			} else {
				retryTime = 0
			}
			await new Promise(resolve => setTimeout(resolve, retryTime))
		}
		console.log('me diz que eu saio dessa coisa pfvr', resultsObj)
		return resultsObj
	}

	evalData = (playsArray, resultsObj) => {
		...
		const updatedPlaysArray = playsArray.map(play => {
			...
			if (targetPlay.win === 'win') {
				...
				const agregated_value = isSoros
					? play.agregated_value = parseFloat(play.agregated_value) + parseFloat(play.agregated_value * 0.8)
					: initialValue
	...
	makePlayFactory = (playsArray) => {
		...
		return async () => {
			while (this.isPlaying && data.length > 0) {
				...
				const winsArray = await this.getWinsArray(data)
				data = this.evalData(data, winsArray)
